...
Sub Bilder_einfügen()
  ...
  RW_Start = 2 'Reihe ab der die Tabelle Startet
  CL_pname = 1 'Spalt in dem der Dateiname für die Bilder steht (in der Regel = Artikelnummer)
  CL_pic = 6   'Spalt in die die Bilder eingefügt werden
  ...
  Rows(RW_Start & ":10000").Select    'Auswahl aller Zeilen von Start bis 10000
  ...
  Columns("F:F").Select         'Auswahl der Bild-Spalte
  Selection.ColumnWidth = 15    'Breite auf 15
   
 
  For i = RW_Start To range_A                                ' Zähler bis zum Ende der Tabelle
    Cells(i, CL_pic).Select                           ' Auswahl der Zelle, in die das Bild soll
    Cells(i, CL_pic).Activate                         ' Aktivierung der Zelle
    picPfad = Pfad & Cells(i, CL_pname) & ".jpg"      ' Bilddateipfad wird aus dem Wert, der in Spalte 4 steht generiert
...
Sub Bilder_loeschen()
    ActiveSheet.Pictures.Delete
     Rows("2:10000").Select    'Auswahl aller Zeilen von 4 bis 10000
  Selection.RowHeight = 13.2   'Zeilenhöhe auf 13,2 (Standard)
End Sub
